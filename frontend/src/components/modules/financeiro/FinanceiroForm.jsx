...
const CATEGORIAS  = ['Venda de Ave', 'Racao', 'Medicamento', 'Equipamento', 'Aneis', 'Manutencao', 'Outros']
const STATUS_OPTS = ['Pago', 'Pendente', 'Cancelado']
const PAGAMENTOS  = ['Dinheiro', 'PIX', 'Cartao', 'Transferencia']

const EMPTY = {
  descricao: '', tipo: 'Receita', categoria: 'Venda de Ave', valor: '',
  data: '', formaPagamento: 'PIX', aveRelacionada: '', notaFiscal: '',
  status: 'Pago', observacoes: '',
}

const FIELDS = [
  { key: 'descricao',      label: 'Descricao',        span: 2 },
  { key: 'tipo',           label: 'Tipo',              type: 'select', opts: TIPOS },
  { key: 'categoria',      label: 'Categoria',         type: 'select', opts: CATEGORIAS },
  { key: 'valor',          label: 'Valor (R$)',        type: 'number' },
  { key: 'data',           label: 'Data',              type: 'date' },
  { key: 'formaPagamento', label: 'Forma de Pagamento',type: 'select', opts: PAGAMENTOS },
  { key: 'status',         label: 'Status',            type: 'select', opts: STATUS_OPTS },
  { key: 'aveRelacionada', label: 'Ave Relacionada',   span: 1 },
  { key: 'notaFiscal',     label: 'Nota Fiscal',       span: 1 },
...
export function FinanceiroForm({ initial, onSave, onClose }) {
    ...
    <div style={S.overlay} onClick={e => e.target === e.currentTarget && onClose()}>
      <div style={S.modal}>
        <div style={{ fontSize: 20, fontWeight: 700, color: '#F2EDE4', marginBottom: 4, fontFamily: "'DM Serif Display', serif" }}>
          {isEdit ? 'Editar Registro Financeiro' : 'Novo Registro Financeiro'}
        </div>
        <div style={{ fontSize: 12, color: '#5A7A5C', fontFamily: "'DM Mono', monospace", marginBottom: 28 }}>
          {isEdit ? `Editando: ${initial.descricao}` : 'Preencha os campos para adicionar um registro'}
        </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, paddingTop: 16, borderTop: '1px solid rgba(255,255,255,0.06)' }}>
          ...
          <button onClick={() => onSave(form)} style={{ background: 'linear-gradient(135deg, #D4A017, #B8870F)', border: 'none', borderRadius: 8, padding: '10px 24px', color: '#0A1A0C', fontSize: 13, fontWeight: 700, fontFamily: "'DM Mono', monospace", cursor: 'pointer' }}>
            {isEdit ? 'Salvar Alteracoes' : 'Cadastrar Registro'}
          </button>
        </div>
      </div>
    </div>

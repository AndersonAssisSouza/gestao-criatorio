...
const CAMPOS = [
  ...
  ['Ocupação',         'ocupacao'],
  ['Aves Residentes',  'avesResidentes'],
  ['Última Limpeza',   'ultimaLimpeza'],
  ['Observações',      'observacoes'],
]

export function GaiolasDetail({ gaiola, onClose, onEdit }) {
  return (
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 20 }}>
        <div>
          ...
          <div style={{ marginTop: 6, display: 'flex', alignItems: 'center', gap: 8 }}>
            <StatusBadge status={gaiola.status} />
            <span style={{ fontSize: 12, color: 'var(--text-muted)', fontFamily: 'inherit' }}>
              {gaiola.tipo} · {gaiola.localizacao}
            </span>
          </div>
        </div>

...
const TIPOS   = ['Voo', 'Choco', 'Reprodução', 'Exposição']
const STATUS  = ['Disponível', 'Ocupada', 'Manutenção']

const EMPTY = {
  codigo: '', tipo: 'Voo', localizacao: '', capacidade: '',
  ocupacao: '', avesResidentes: '', status: 'Disponível',
  ultimaLimpeza: '', observacoes: '',
}

const FIELDS = [
  { key: 'codigo',         label: 'Código da Gaiola',  span: 1 },
  { key: 'tipo',           label: 'Tipo',               type: 'select', opts: TIPOS },
  { key: 'localizacao',    label: 'Localização',        span: 2 },
  { key: 'capacidade',     label: 'Capacidade',         type: 'number' },
  { key: 'ocupacao',       label: 'Ocupação Atual',     type: 'number' },
  { key: 'status',         label: 'Status',             type: 'select', opts: STATUS },
  { key: 'ultimaLimpeza',  label: 'Última Limpeza',     type: 'date' },
  { key: 'avesResidentes', label: 'Aves Residentes',    span: 2 },
...
export function GaiolasForm({ initial, onSave, onClose }) {
    ...
    <div style={S.overlay} onClick={e => e.target === e.currentTarget && onClose()}>
      <div style={S.modal}>
        <div style={{ fontSize: 20, fontWeight: 700, color: '#F2EDE4', marginBottom: 4, fontFamily: "'DM Serif Display', serif" }}>
          {isEdit ? 'Editar Gaiola' : 'Cadastrar Nova Gaiola'}
        </div>
        <div style={{ fontSize: 12, color: '#5A7A5C', fontFamily: "'DM Mono', monospace", marginBottom: 28 }}>
          {isEdit ? `Editando: ${initial.codigo}` : 'Preencha os campos para adicionar uma gaiola'}
        </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, paddingTop: 16, borderTop: '1px solid rgba(255,255,255,0.06)' }}>
          ...
          <button onClick={() => onSave(form)} style={{ background: 'linear-gradient(135deg, #C95025, #A0401D)', border: 'none', borderRadius: 8, padding: '10px 24px', color: '#0A1A0C', fontSize: 13, fontWeight: 700, fontFamily: "'DM Mono', monospace", cursor: 'pointer' }}>
            {isEdit ? 'Salvar Alterações' : 'Cadastrar Gaiola'}
          </button>
        </div>
      </div>
    </div>

...
const STATUS     = ['Em Choco', 'Eclodido', 'Abandonado']

const EMPTY = {
  aveNome: '', categoriaAve: 'Canário', gaiola: '',
  dataInicio: '', previsaoEclosao: '',
  quantidadeOvos: '', ovosFerteis: '',
  status: 'Em Choco', observacoes: '',
}

const FIELDS = [
  { key: 'aveNome',         label: 'Nome da Ave',        span: 2 },
  ...
  { key: 'previsaoEclosao', label: 'Previsão Eclosão',    type: 'date' },
  { key: 'quantidadeOvos',  label: 'Quantidade de Ovos',  type: 'number' },
  { key: 'ovosFerteis',     label: 'Ovos Férteis',        type: 'number' },
  { key: 'status',          label: 'Status',              type: 'select', opts: STATUS },
  { key: 'observacoes',     label: 'Observações',         span: 2 },
]
...
export function ChocandoForm({ initial, onSave, onClose }) {
  const [form, setForm] = useState(initial ? { ...initial, quantidadeOvos: String(initial.quantidadeOvos ?? ''), ovosFerteis: String(initial.ovosFerteis ?? '') } : EMPTY)
  ...
  const handleSubmit = () => {
    const parsed = {
      ...form,
      quantidadeOvos: form.quantidadeOvos !== '' ? Number(form.quantidadeOvos) : 0,
      ovosFerteis:    form.ovosFerteis    !== '' ? Number(form.ovosFerteis)    : 0,
    }
    onSave(parsed)
  }

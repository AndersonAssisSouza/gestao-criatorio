...
const CAMPOS = [
  ...
  ['Previsão Eclosão',   'previsaoEclosao'],
  ['Quantidade de Ovos', 'quantidadeOvos'],
  ['Ovos Férteis',       'ovosFerteis'],
  ['Categoria',          'categoriaAve'],
  ['Status',             'status'],
  ['Observações',        'observacoes'],
]

export function ChocandoDetail({ registro, onClose, onEdit }) {
  return (
        ...
        <div>
          ...
          <div style={{ marginTop: 6, display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <span style={{ fontSize: 12, color: 'var(--text-muted)', fontFamily: 'inherit' }}>
              {registro.categoriaAve} · Gaiola {registro.gaiola || '—'}
            </span>
          </div>
        </div>
        ...
        {CAMPOS.map(([label, key]) => (
          <div key={key}>
            ...
            <div style={{ fontSize: 13, color: '#D8E8D8', fontFamily: 'inherit' }}>
              {key === 'status' ? <StatusBadge status={registro[key]} /> : (registro[key] ?? '—')}
            </div>
          </div>
        ))}
      </div>

...
export function AneisTable({ data, onRowClick, onEdit, onDelete }) {
  return (
    <table style={{ width: '100%', borderCollapse: 'collapse' }}>
      ...
      <tbody>
        {data.map(row => (
          ...
          >
            <TD highlight>{row.codigo}</TD>
            <TD>{row.tipo}</TD>
            <TD>{row.diametro ? `${row.diametro} mm` : '\u2014'}</TD>
            <TD>{row.cor || '\u2014'}</TD>
            <TD>{row.aveAssociada || '\u2014'}</TD>
            <TD>{row.dataColocacao || '\u2014'}</TD>

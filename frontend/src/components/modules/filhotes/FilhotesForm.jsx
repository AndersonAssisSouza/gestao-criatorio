...
const EMPTY = {
  nome: '', categoriaAve: 'Canário', dataNascimento: '',
  nomeMae: '', nomePai: '', gaiola: '', anelamento: '',
  peso: '', status: 'Em Desenvolvimento', destino: '-',
  observacoes: '',
}

const FIELDS = [
  { key: 'nome',           label: 'Nome / ID Temporário', span: 2 },
  ...
  { key: 'anelamento',     label: 'Anelamento' },
  { key: 'peso',           label: 'Peso (g)',              type: 'number' },
  { key: 'destino',        label: 'Destino',               type: 'select', opts: DESTINOS },
...
const S = {
  ...
  textarea:{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 8, padding: '10px 12px', color: '#F2EDE4', fontSize: 13, fontFamily: "'DM Mono', monospace", outline: 'none', width: '100%', boxSizing: 'border-box', minHeight: 70, resize: 'vertical' },
}

export function FilhotesForm({ initial, onSave, onClose }) {
  const [form, setForm] = useState(initial ? { ...initial, peso: initial.peso != null ? String(initial.peso) : '' } : EMPTY)
  const set = (k, v) => setForm(f => ({ ...f, [k]: v }))
  const isEdit = !!initial?.id

  const handleSubmit = () => {
    const payload = { ...form, peso: form.peso !== '' ? Number(form.peso) : 0 }
    onSave(payload)
  }
    ...
    <div style={S.overlay} onClick={e => e.target === e.currentTarget && onClose()}>
      <div style={S.modal}>
        <div style={{ fontSize: 20, fontWeight: 700, color: '#F2EDE4', marginBottom: 4, fontFamily: "'DM Serif Display', serif" }}>
          {isEdit ? 'Editar Filhote' : 'Cadastrar Novo Filhote'}
        </div>
        <div style={{ fontSize: 12, color: '#5A7A5C', fontFamily: "'DM Mono', monospace", marginBottom: 28 }}>
          {isEdit ? `Editando: ${initial.nome}` : 'Preencha os campos para registrar o filhote'}
        </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, paddingTop: 16, borderTop: '1px solid rgba(255,255,255,0.06)' }}>
          ...
          <button onClick={handleSubmit} style={{ background: 'linear-gradient(135deg, #D4A017, #B8870F)', border: 'none', borderRadius: 8, padding: '10px 24px', color: '#0A1A0C', fontSize: 13, fontWeight: 700, fontFamily: "'DM Mono', monospace", cursor: 'pointer' }}>
            {isEdit ? 'Salvar Alterações' : 'Cadastrar Filhote'}
          </button>
        </div>
      </div>
    </div>

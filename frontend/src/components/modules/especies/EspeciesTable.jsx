...
export function EspeciesTable({ data, onRowClick, onEdit, onDelete }) {
  return (
      ...
      <tbody>
        {data.map(row => (
          ...
          >
            ...
            <TD>{row.familia}</TD>
            <TD>
              <span style={{ fontWeight: 600, color: '#C95025' }}>{row.quantidadePlantel}</span>
            </TD>
            <TD>{row.corPredominante}</TD>
            <TD>{row.tamanhoMedio ? `${row.tamanhoMedio} cm` : '—'}</TD>
            <TD><StatusBadge status={row.statusConservacao} /></TD>

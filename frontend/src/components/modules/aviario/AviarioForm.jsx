...
const TIPOS       = ['Sala Interna', 'Viveiro Externo', 'Quarentena']
const STATUS      = ['Ativo', 'Inativo', 'Reforma']
const ILUMINACOES = ['Natural', 'Artificial', 'Mista']

const EMPTY = {
  nome: '', tipo: 'Sala Interna', area: '', capacidadeGaiolas: '',
  gaiolasInstaladas: '', temperatura: '', umidade: '', iluminacao: 'Natural',
  status: 'Ativo', ultimaManutencao: '', observacoes: '',
}

const FIELDS = [
  { key: 'nome',              label: 'Nome do Espaco',         span: 1 },
  { key: 'tipo',              label: 'Tipo',                   type: 'select', opts: TIPOS },
  { key: 'area',              label: 'Area (m2)',              type: 'number' },
  { key: 'capacidadeGaiolas', label: 'Capacidade de Gaiolas',  type: 'number' },
  { key: 'gaiolasInstaladas', label: 'Gaiolas Instaladas',     type: 'number' },
  { key: 'temperatura',       label: 'Temperatura Media (°C)', type: 'number' },
  { key: 'umidade',           label: 'Umidade Media (%)',      type: 'number' },
  { key: 'iluminacao',        label: 'Iluminacao',             type: 'select', opts: ILUMINACOES },
  { key: 'status',            label: 'Status',                 type: 'select', opts: STATUS },
  { key: 'ultimaManutencao',  label: 'Ultima Manutencao',      type: 'date' },
...
export function AviarioForm({ initial, onSave, onClose }) {
    ...
    <div style={S.overlay} onClick={e => e.target === e.currentTarget && onClose()}>
      <div style={S.modal}>
        <div style={{ fontSize: 20, fontWeight: 700, color: 'var(--text-main)', marginBottom: 4, fontFamily: "'DM Serif Display', serif" }}>
          {isEdit ? 'Editar Espaco' : 'Cadastrar Novo Espaco'}
        </div>
        <div style={{ fontSize: 12, color: 'var(--text-muted)', fontFamily: 'inherit', marginBottom: 28 }}>
          {isEdit ? `Editando: ${initial.nome}` : 'Preencha os campos para adicionar um espaco ao aviario'}
        </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10, paddingTop: 16, borderTop: '1px solid rgba(255,255,255,0.06)' }}>
          ...
          <button onClick={() => onSave(form)} style={{ background: 'linear-gradient(135deg, #C95025, #A0401D)', border: 'none', borderRadius: 8, padding: '10px 24px', color: '#0A1A0C', fontSize: 13, fontWeight: 700, fontFamily: 'inherit', cursor: 'pointer' }}>
            {isEdit ? 'Salvar Alteracoes' : 'Cadastrar Espaco'}
          </button>
        </div>
      </div>
    </div>
